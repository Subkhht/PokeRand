...
export function resolvePvpTurn(
  ...
  if (state.phase === 'switch') {
    ...
    for (const side of switchSides) {
      ...
      const ps = side === 'a' ? a : b
      if (action.index >= 0 && action.index < ps.team.length && ps.team[action.index].hp > 0) {
        const incoming = ps.team[action.index]
        ps.active = action.index
        ps.revealed[action.index] = true
        ps.team[action.index] = {
          ...incoming,
        ...
        log.push(`🔁 ${who} saca a ${incoming.name}.`)
        changed = true
      }
    }
    if (!changed) return { state, changed: false }
    return { state: { ...state, phase: 'picking', switchFor: null, log, a, b }, changed: true }
  }
  ...
  if (
    !activeA || !activeB ||
    !actionA || actionA.kind !== 'move' ||
    !actionB || actionB.kind !== 'move'
  ) {
  ...
  if (second) applyMove(second, second === 'a' ? 'b' : 'a')
  ...
  const nextLog = log.slice(-50)

  return {
    state: { ...state, phase, winner, switchFor, log: nextLog, a, b },
    changed: true,
  }
}
