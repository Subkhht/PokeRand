...
export function scalePokemonForNode(base: Pokemon, _node: RouteNode, _stepIndex: number, levelDelta = 0, difficulty: string = 'medium'): Pokemon {
  const hpMultiplier = difficulty === 'easy' ? 3 : difficulty === 'infinite' ? 5.2 : difficulty === 'hard' ? 5 : difficulty === 'coliseum' ? 5 : difficulty.startsWith('aventura') ? 4.6 : 4
  const statMultiplier = difficulty === 'easy' ? 1.5 : difficulty === 'infinite' ? 2.55 : difficulty === 'hard' ? 2.5 : difficulty === 'coliseum' ? 2.5 : difficulty.startsWith('aventura') ? 2.3 : 2
...
export function balanceWildPokemonToTeam(enemy: Pokemon, team: Pokemon[], difficulty: string): Pokemon {
  ...
  const strength = difficulty === 'easy' ? 0.72 : difficulty === 'infinite' ? 1.02 : difficulty === 'hard' ? 1.05 : difficulty.startsWith('aventura') ? 1.0 : 0.95
